...
import { OpenAIProvider } from "@/lib/coach/provider";
...
function describeResult(result: ReturnType<typeof executeAction>): string {
  ...
  if (result.kind === "explanation") {
    ...
    if (exercise == null) {
      return "I couldn't find that exercise right now.";
    }
    ...
    return `${exercise.name}: no detailed instructions available.`;
  }
...
export async function POST(request: Request) {
  ...
  if (!reply) {
    ...
    if (canUse === false) {
      reply =
        "You've reached today's coaching limit. Please try again tomorrow.";
    } else {
      try {
        const [
          { data: profile },
        ...
        if (!profile || !prefs) {
          reply =
            "Please complete onboarding before using the coach.";
        } else {
          const context: WorkoutInput = {
            ...
            workout_location:
              prefs.workout_location as WorkoutInput["workout_location"],
          ...
          const raw = await provider.respond({
            ...
            context: {
              ...context,
              exercise_catalog: EXERCISE_CATALOG.map(
                (exercise) => ({
                  id: exercise.id,
                  name: exercise.name,
                  movement_pattern:
